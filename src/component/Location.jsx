      ...
      <View style={styles.container}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={{ color: '#b5895d', fontWeight: 'bold', fontSize: 16 } }> / Locations</Text>
          </View>
        ...
        {pageData.length === 0 ? (
          ...
          pageData.map((item, index) => (
            <View key={item._id} style={styles.row}>
              <Text style={[styles.cell, { flex: 1}]}> {(currentPage - 1) * itemsPerPage + (index + 1)} </Text>
              <Text style={[styles.cell, { flex: 2 }]}>{item.location}</Text>
              <TouchableOpacity
                style={[styles.deleteBtn,{flex:.8}]}
                onPress={() => {
