...
const PropertyFilter = ({
  ...
  const FetchAmenity = async () => {
    try {
      const resAmenity = await fetch(
        `http://${myurl.BASE_URL}/api/admin/amenities`,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `http://${myurl.BASE_URL}/api/admin/filterproperty`,
        {
          method: 'POST',
  ...
  return (
    <ScrollView
      style={[styles.container, !getToggleFilter && {display: 'flex'}]}>
      <TouchableOpacity
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity
          ...
          <Text style={getany ? styles.typeBtnTextActive : styles.typeBtnText}>
            Any Type
          </Text>
        </TouchableOpacity>
        {['Apartment', 'Plot', 'Condos', 'Villa', 'Family', 'Single Room'].map(
          type => {
            ...
            return (
              <TouchableOpacity
                ...
                <Text
                  style={
                    isSelected ? styles.typeBtnTextActive : styles.typeBtnText
                  }>
                  {type}
                </Text>
              </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              handleChange(
                'options',
                {
                  val: option.amenities,
                  checked: !filters.options.includes(option.amenities),
                },
                'checkbox',
              )
            }>
            <View
              style={[
                styles.checkbox,
                filters.options.includes(option.amenities) &&
