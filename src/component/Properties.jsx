...
import Pagination from "./Pagination"; // Adapted for React Native
...
//   const navigation = useNavigation();
  ...
  const fetchProperties = async () => {
    try {
      const response = await fetch(`http://${perpage.BASE_URL}/api/admin/getproperties`, {
        method: "GET",
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.row}>
      ...
      <Text style={[styles.cell,{textTransform:'capitalize'}]}>{item.title}</Text>
      <Text style={[styles.cell,{textTransform:'capitalize'}]}>{item.builder_name}</Text>
      <Text style={[styles.cell,{textTransform:'capitalize'}]}>{item.location}</Text>
      <View style={[styles.cell, { flexDirection: "row" }]}>
        <TouchableOpacity
          onPress={() => navigation.navigate("EditProperty", { id: item._id })}
        >
          <Text style={styles.edit}><Icon name="edit" size={18} color="#aa8453" /></Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={{ marginLeft: 16 }}
          onPress={() => {
        ...
        >
          <Text style={styles.delete}><Icon name="trash" size={18} color="#aa8453" /></Text>
        </TouchableOpacity>
      </View>
    </View>
  );
      ...
      <View style={styles.container}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TouchableOpacity style={styles.searchButton} onPress={mySubmit}>
            <Text style={styles.searchText}><Icon name="search" size={20} color="#fff" /></Text>
          </TouchableOpacity>
        </View>
        {/* Table Header */}
...
const styles = StyleSheet.create({
  ...
  confirmText: { color: "#fff" ,paddingHorizontal: 5},
  cancelButton: {
    backgroundColor: "#333",
    padding: 10,
    borderRadius: 5,
